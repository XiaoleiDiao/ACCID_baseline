...
from nets.DAL import DSAL_layer
...
def RSEBn(pretrained, **kwargs):
    model = RSEB([1, 2, 8, 8, 4])
    if pretrained:
...
class RSEBody(nn.Module):
    def __init__(self, config):
        super(RSEBody, self).__init__()
        self.config = config
        self.backbone = RSEBn(None)
        ...
        #------------------------------------------------------------------------#
        final_out_filter0 = len(config["RSE"]["anchors"][0]) * (5 + config["RSE"]["classes"])   # final_out_filter0 = 192
        final_out_filter1 = int(config["RSE"]["s_classes"])  # final_out_filter1 = 6, the number of the structural relations
